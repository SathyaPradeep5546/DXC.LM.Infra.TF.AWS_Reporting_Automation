...
data "archive_file" "zip_the_python_code_health_events" {
type        = "zip"
source_file  = "${path.module}/Python-Scripts/aws-health-events.py"
output_path = "${path.module}/Python-Scripts/Python-Scripts-Archives/aws-health-events.zip"
}

###################################################################################
# Creating AWS Lamda Function for AWS Health Events Reports

resource "aws_lambda_function" "aws-health-events-lambda" {
  count              = var.cdeploycentralonly == true ? 1 : 0
  source_code_hash = data.archive_file.zip_the_python_code_health_events.output_base64sha256
  description = "Function to create central AWS Health Events Reports"
  filename      = "${path.module}/Python-Scripts/Python-Scripts-Archives/aws-health-events.zip" 
  function_name = "LF-CENTRAL-AWS-HEALTH-EVENTS-SVC-LM"
  role          = aws_iam_role.Reporting_Automation_Lambda_role[count.index].arn
  handler       = "aws-health-events.lambda_handler"
  memory_size = 300
  timeout = 900
  runtime = "python3.10"
  tags = merge(
    {map-migrated = null},
    var.aws_health_events_lambda_tags
  )
